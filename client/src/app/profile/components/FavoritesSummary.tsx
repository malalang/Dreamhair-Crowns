    ...
    return (
        <Section>
            <div className="flex items-center justify-between mb-4">
                ...
                <Link href="/shop" className="text-sm text-yellow-400 hover:text-white transition-colors flex items-center gap-1">
                    Browse Shop <HiChevronRight />
                </Link>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
                {favorites.map((fav: any) => {
                    ...
                    return (
                        <Link
                            href={`/shop/${product.category_name}/${product.slug || product.id}`}
                            key={product.id}
